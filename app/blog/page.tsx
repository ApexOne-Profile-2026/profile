          ...
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {posts.map((post, index) => (
              <FadeIn key={post.id} delayMs={index * 70} as="article">
                <article className="card-surface group flex h-full flex-col overflow-hidden rounded-[1.5rem] transition-[transform,border-color,box-shadow] duration-300 hover:-translate-y-1 hover:border-accent/25 hover:shadow-[0_28px_60px_-36px_rgba(15,23,42,0.35)]">
                  <Link href={`/blog/${post.slug}`} className="flex h-full flex-col">
                    ...
                    >
                      <div className="absolute inset-x-4 bottom-4">
                        <time
                          dateTime={post.date}
                          className="inline-flex rounded-full border border-border bg-background/80 px-3 py-1 text-xs font-medium text-accent backdrop-blur-md"
                        >
