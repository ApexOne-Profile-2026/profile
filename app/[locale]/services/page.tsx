...
export async function generateMetadata({
  ...
  return {
    ...
    description: dict.pages.services.heroDescription,
  };
}
...
export default async function ServicesPage({
  ...
  const localizedServices = services.map((service) =>
    getLocalizedService(service, locale as Locale),
  );

  return (
    <main className="flex flex-1 flex-col">
      <PageHero
        eyebrow={dict.pages.services.title}
        title={dict.pages.services.heroTitle}
      >
        <div className="flex flex-col gap-3 sm:flex-row">
          <Link
            href={`${localizeHref(locale as Locale, "/contact")}?intent=consultation`}
            className="btn-primary h-11 px-6"
          >
            {dict.pages.services.consultation}
          </Link>
          <Link
            href={localizeHref(locale as Locale, "/products")}
            className="btn-secondary h-11"
          >
            {dict.pages.services.seeProducts}
          </Link>
        </div>
      </PageHero>
              ...
              <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                <div className="max-w-2xl">
                  <p className="font-display text-sm font-semibold tracking-[0.18em] text-accent-secondary uppercase">
                    {dict.pages.services.engagement}
                  </p>
                  <h2 className="mt-3 font-display text-xl font-semibold tracking-[-0.03em] sm:text-2xl">
                    {dict.pages.services.engagementTitle}
                  </h2>
                  <p className="mt-4 text-sm leading-relaxed text-white/70 sm:text-base">
                    {dict.pages.services.engagementDescription}
                  </p>
                </div>
                <Link
                  href={`${localizeHref(locale as Locale, "/contact")}?intent=consultation`}
                ...
                >
                  {dict.pages.services.engagementCta}
                </Link>
              </div>
            </div>
          </FadeIn>
