...
export default async function ProductPage({ params }: ProductPageProps) {
  ...
  const related = products
    ...
    .slice(0, 2)
    .map((item) => getLocalizedProduct(item, "en"));
              ...
              {related.map((item, relatedIndex) => (
                <FadeIn key={item.id} delayMs={relatedIndex * 80}>
                  <Link
                    ...
                    className="card-surface group overflow-hidden rounded-[1.35rem] transition-[transform,border-color,box-shadow] duration-300 hover:-translate-y-0.5 hover:border-accent/25 hover:shadow-[0_24px_50px_-36px_rgba(15,23,42,0.35)]"
                  >
